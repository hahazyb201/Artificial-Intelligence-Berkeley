...
def visit(pic,i,j):
    pic[i][j]=1
    if i-1>=0 and pic[i-1][j]==0:
        visit(pic,i-1,j)
    if i+1<len(pic) and pic[i+1][j]==0:
        visit(pic,i+1,j)
    if j-1>=0 and pic[i][j-1]==0:
        visit(pic,i,j-1)
    if j+1<len(pic[0]) and pic[i][j+1]==0:
        visit(pic,i,j+1)
    return
def enhancedFeatureExtractorDigit(datum):
    ...
    picture=datum.getPixels()
    cp=copy.deepcopy(picture)
    ...
    features[1]=1.0 if numOfIslands==1 else 0.0
    features[2]=1.0 if numOfIslands==2 else 0.0
    features[3]=1.0 if numOfIslands==3 else 0.0
    features[4]=1.0 if numOfIslands>3 else 0.0
   
    #Adding any other features will cause overfitting!!!
...
def enhancedPacmanFeatures(state, action):
    ...
    for i in range(len(food[:])):
        for j in range(len(food[0])):
            if food[i][j]:
                dis=util.manhattanDistance((i,j),nextPos)
                minDis=min(dis,minDis)
    minGhostDis=float("inf")

    for ghost in successor.getGhostStates():
        minGhostDis=min(minGhostDis,util.manhattanDistance(ghost.getPosition(),nextPos))
    features['nearestGhost']=minGhostDis
    features['nearestFood']=minDis
    return features
