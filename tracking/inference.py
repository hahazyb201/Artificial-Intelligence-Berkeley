...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        priorPredict=util.Counter()
    
        for oldPos in self.legalPositions:
            newPosDist=self.getPositionDistribution(self.setGhostPosition(gameState,oldPos))
            for pos in newPosDist:
                priorPredict[pos]+=newPosDist[pos]*self.beliefs[oldPos]

        priorPredict.normalize()
        self.beliefs=priorPredict
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance==None:
            ...
            for p in self.particles:
                ...
                self.beliefs[p]+=emissionModel[dis]
    ...
    def observeState(self, gameState):
        ...
        for g in range(self.numGhosts):
            if noisyDistances[g]==None:
                ghostInJail.append(g)

        newParticles=util.Counter()
        for p in self.particles:
            for g in ghostInJail:
                p=self.getParticleWithGhostInJail(p,g)
            prob=1.0
            for i in range(self.numGhosts):
                if i not in ghostInJail:
                    dis=util.manhattanDistance(p[i],pacmanPosition)
                    prob*=emissionModels[i][dis]
